    ...
    <>
      ...
      {isModalOpen && (
        <ModalForm 
          user = {selectedUser}
          isOpen={isModalOpen}
          onClose={() => {setModalOpen(false), setSelectedUser(undefined)}}
        />
      )
      }
        <MainTable
          users = {users}
          onUserClick = {(id) => {setModalOpen(true); setSelectedUser(userById(id))}}
          filter = {filterAbsense}
          search = {searchFilter}
          />

      <div className={`fixed top-0 left-0 w-[100vw] h-[100vh] bg-[#00000030] absolute z-10 ${!isModalOpen && "hidden"}`}></div>
    </>
  )
}
